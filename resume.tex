...
\textbf{기술 스택}: \tech{Java, Spring MVC, MyBatis, JSP, MariaDB, NHN KCP, Biztalk, Quartz, 카카오맵 API, 사이렌 휴대폰 인증}  

\textbf{문제/해결/성과}:
\begin{itemize}[leftmargin=*]
  \item \textbf{문제:} 피크 시간대 동시 예약 시 오버부킹·결제 중복·승인 지연 발생 가능성.
  \item \textbf{해결:} DB 행 잠금 + 백오프 기반 재시도 로직 적용. NHN KCP 콜백 검증 및 트랜잭션 분리로 이중결제 차단. 예약 승인/취소·환불 절차 표준화.  
